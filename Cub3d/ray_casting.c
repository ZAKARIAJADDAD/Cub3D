...
/*   Updated: 2023/09/17 17:37:08 by zael-wad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	clac_projextion_distance(t_2var *data)
{
	...
	((data->x_screen / 2) / tan(convert_dgree(30)));
}
...
void	virtical_mapping(t_2var	*data)
{
	...
	if (cos(data->player_pos.ray_angle) > 0)
	{
		...
		get_colors(&data->textuer2, k, data->dda_data.y_tex);
	}
	else
	{
		data->dda_data.y_tex *= \
		...
		data->dda_data.texture_color = \
		get_colors(&data->textuer3, k, data->dda_data.y_tex);
	}
}
...
void	horizontal_mapping(t_2var *data)
{
	...
	if (sin(data->player_pos.ray_angle) > 0)
	{
		...
		data->dda_data.texture_color = \
		get_colors(&data->textuer, k, data->dda_data.y_tex);
	}
	else
	{
		data->dda_data.y_tex *= \
		...
		data->dda_data.texture_color = \
		get_colors(&data->textuer1, k, data->dda_data.y_tex);
	}
}
